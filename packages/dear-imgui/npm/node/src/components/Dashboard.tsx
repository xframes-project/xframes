...
export const Dashboard = () => {
  ...
  useEffect(() => {
    canvasRef.current?.setScriptFile("./src/scripts/drawing-primitives.js");
  }, []);

  // Canvas: data-driven bar chart with live updates
  useEffect(() => {
    dataCanvasRef.current?.setScriptFile("./src/scripts/bar-chart.js");
  ...
  useEffect(() => {
    ctx2dCanvasRef.current?.setScriptFile("./src/scripts/analog-clock.js");

    const now = new Date();
    let elapsed = now.getHours() * 3600 + now.getMinutes() * 60 + now.getSeconds();
    ctx2dCanvasRef.current?.setData({ time: elapsed });
  ...
  useEffect(() => {
    luaClockRef.current?.setScriptFile("./src/scripts/lua-analog-clock.lua");
  ...
  useEffect(() => {
    janetCanvasRef.current?.setScriptFile("./src/scripts/janet-drawing-primitives.janet");
  }, []);

  // JanetCanvas: ctx 2D API demo — analog clock using Janet Canvas 2D API
  useEffect(() => {
    janetClockRef.current?.setScriptFile("./src/scripts/janet-analog-clock.janet");
