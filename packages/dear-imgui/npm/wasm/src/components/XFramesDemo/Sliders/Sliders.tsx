...
export const Sliders = () => {
  ...
  const [doubleSliderValue, setDoubleSliderValue] = useState<[number, number]>([
    0, 0,
  ]);
  const [tripleSliderValue, setTripleSliderValue] = useState<
    [number, number, number]
  >([0, 0, 0]);
  const [quadSliderValue, setQuadSliderValue] = useState<
    [number, number, number, number]
  >([0, 0, 0, 0]);

  const handleSingleSliderValueChanged = useCallback(
  ...
  const handleAngleSliderValueChanged = useCallback(
    (event: SliderChangeEvent) => {
      if (event.nativeEvent) {
  ...
  const handleDoubleSliderValueChanged = useCallback(
    (event: MultiSliderChangeEvent<number>) => {
      if (event.nativeEvent) {
  ...
  const handleTripleSliderValueChanged = useCallback(
    (event: MultiSliderChangeEvent<number>) => {
      if (event.nativeEvent) {
  ...
  const handleQuadSliderValueChanged = useCallback(
    (event: MultiSliderChangeEvent<number>) => {
      if (event.nativeEvent) {
        setQuadSliderValue([
          event.nativeEvent.values[0],
